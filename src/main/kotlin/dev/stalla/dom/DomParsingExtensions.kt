...
/**
 * Parses the node contents into a [ItunesStyleCategoryBuilder2] if possible, ensuring the child nodes
 * have the specified [namespace], then populates the [categoryBuilder] with the parsed data.
 *
 * @param categoryBuilder An empty [ItunesStyleCategoryBuilder2] instance to initialise with the node's
 * contents.
 * @param namespace The [FeedNamespace] to ensure the child nodes have.
 *
 * @return The [categoryBuilder] populated with the DOM node contents.
 */
@InternalApi
internal fun Node.toITunesCategoryBuilder(categoryBuilder: ItunesStyleCategoryBuilder2, namespace: FeedNamespace? = null): ItunesStyleCategoryBuilder2 {
